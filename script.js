...
function renderSocialLinks(instructor) {
  ...
  // TODO: Update social links when newer official profiles are provided.
}
...
function renderLectures() {
  ...
  const cards = lectures.map((lecture) => {
    ...
    const title = createElement("h3", null, lecture.displayTitle);
...
function renderTextbooks(textbooks) {
  const items = textbooks.map((book) => {
    ...
    if (book.links.length) {
      ...
      links.append(...book.links.map((link) => externalLink(link.label, link.url)));
      item.append(links);
    }
    return item;
  });
...
function resourceLink(resource) {
  ...
  const link = createElement("a", "resource-link");
...
function renderResources(resources) {
  ...
  list.append(...resources.map(resourceLink));
  selectors.resourceList.replaceChildren(list);
}
...
async function loadCourse() {
  ...
  const response = await fetch("data/course.json?v=20260624-eero-linear-algebra");
  if (!response.ok) {
